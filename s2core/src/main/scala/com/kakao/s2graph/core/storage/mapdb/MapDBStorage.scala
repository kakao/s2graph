...
object MapDBStorage {
  ...
  def makeClient(config: Config, overrideKv: (String, String)*) = MapDB.client
}

class MapDBStorage(val config: Config, vertexCache: Cache[Integer, Option[Vertex]])
  ...
  val client = MapDBStorage.makeClient(config)
  val queryBuilder = new MapDBQueryBuilder(this)(ec)
  val mutationBuilder = new MapDBMutationBuilder(this)(ec)

//  val cacheOpt = Option(cache)
  val cacheOpt = None
  val vertexCacheOpt = Option(vertexCache)

  private val clientWithFlush = MapDBStorage.makeClient(config, "hbase.rpcs.buffered_flush_interval" -> "0")
  ...
  private def writeToStorage(_client: MapDBStorageClient, rpc: HBaseRpc): Deferred[Boolean] = {
    //    logger.debug(s"$rpc")
    val defer = rpc match {
