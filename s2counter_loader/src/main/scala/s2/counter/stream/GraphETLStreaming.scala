...
import s2.counter.{CounterETLFunctions, CounterETLItem}
import s2.models.{CounterModel, DBModel, DimensionProps}
...
/**
  ...
  */
object GraphETLStreaming extends SparkApp with WithKafka {
    ...
    stream.foreachRDD { rdd =>
      rdd.foreachPartitionWithOffsetRange { case (osr, part) =>
        ...
        val joinItems = items.toList.groupBy { e =>
          ...
          CounterETLFunctions.checkPolicyAndJoinUserProfile(service, action, v)
        }
        ...
        log.warn(DimensionProps.getStatsString)
