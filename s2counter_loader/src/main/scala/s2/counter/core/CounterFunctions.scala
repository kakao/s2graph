...
//  private[counter] val ttlMap: Map[String, Int] = Map("m" -> 3600 * 2, "H" -> 3600 * (1 + 24 * 2), "d" -> 3600 * (24 + 24 * 30))
  ...
  /**
   * transform transaction log of exact counter to ranking counter item
   * @param log transaction log of exact counter
   * @return sequence of ranking key and item's score
   */
  def rankingMapper(log: TrxLog): Seq[(RankingKey, RankingValueMap)] = {
    // normal ranking
    for {
      policy <- DefaultCounterModel.findById(log.policyId).toSeq
      result <- log.results
    } yield {
      (RankingKey(policy.id, policy.version, ExactQualifier(TimedQualifier(result.interval, result.ts), result.dimension)),
        Map(log.item -> RankingValue(result.result, result.value)))
    }
  }
  ...
  def logToExact(logs: Iterator[TrxLog]): Iterator[ExactCounterRow] = {
    for {
      log <- logs
      item <- logToExact(log)
    } yield {
  ...
  def logToExact(log: TrxLog): Option[ExactCounterRow] = {
    for {
      policy <- DefaultCounterModel.findById(log.policyId)
    } yield {
      ...
      val value = {
        for {
          ...
          ExactQualifier(TimedQualifier(result.interval, result.ts), result.dimension) -> result.result
        }
      }.toMap
      ExactCounterRow(key, value)
    }
  }
...
//      baseFilled.foreach(x => println(s"base filled>> $x"))
  ...
  def updateRankingCounter(values: TraversableOnce[(RankingKey, RankingValueMap)], acc: HashMapAccumulable): Unit = {
    for {
      (key, value) <- values
      rankingCounter <- rankingCounterByVersion(key.version)
    } {
  ...
  def getRankingCounter(key: RankingKey): Option[RankingResult] = {
    for {
      rankingCounter <- rankingCounterByVersion(key.version)
      result <- rankingCounter.getTopK(key)
    } yield {
      result
    }
  }

  def deleteRankingCounter(key: RankingKey): Unit = {
    for {
      rankingCounter <- rankingCounterByVersion(key.version)
    } {
